...
exports.KickAction = helpers_1.automodAction({
    configType: t.type({
    ...
    async apply({ pluginData, contexts, actionConfig, matchResult }) {
        ...
        const userIdsToKick = utils_1.unique(contexts.map(c => c.user?.id).filter(utils_1.nonNullish));
        const membersToKick = await utils_1.asyncMap(userIdsToKick, id => utils_1.resolveMember(pluginData.client, pluginData.guild, id));
