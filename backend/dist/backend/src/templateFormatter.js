...
exports.renderTemplate = exports.renderParsedTemplate = exports.parseTemplate = exports.TemplateParseError = void 0;
    ...
    const dumpArg = () => {
        ...
        if (currentVar._state.currentArgType) {
            if (currentVar._state.currentArgType === "number") {
                if (isNaN(currentVar._state.currentArg)) {
                    throw new TemplateParseError(`Invalid numeric argument: ${currentVar._state.currentArg}`);
                }
                currentVar.args.push(parseFloat(currentVar._state.currentArg));
            }
            else {
...
async function evaluateTemplateVariable(theVar, values) {
    const value = utils_1.has(values, theVar.identifier) ? utils_1.get(values, theVar.identifier) : undefined;
    if (typeof value === "function") {
        ...
        for (const arg of theVar.args) {
            if (typeof arg === "object") {
                const argValue = await evaluateTemplateVariable(arg, values);
                args.push(argValue);
            }
            else {
                args.push(arg);
...
async function renderParsedTemplate(parsedTemplate, values) {
    let result = "";
...
exports.renderParsedTemplate = renderParsedTemplate;
const baseValues = {
    ...
    rand(from, to, seed = null) {
        ...
        const randValue = seed != null ? new seedrandom_1.default(seed)() : Math.random();
...
async function renderTemplate(template, values = {}, includeBaseValues = true) {
